    ...
    if (arguments.length > 0) {
        ...
        if (arg instanceof Buffer) {
            if (arguments.length != 1) {
                new Error("APDU " + APDU.SW_GENERALERROR + " Only one argument of type Buffer expected");
            }
            this.fromByteString(arg);
        } else {
            if ((arguments.length < 4) || (arguments.length > 6)) {
                new Error("APDU " + APDU.SW_GENERALERROR + "4 to 6 arguments expected");
            }

            for (var i = 0; i < 4; i++) {
                if (typeof(arguments[i]) != "number") {
                    new Error("APDU" + APDU.SW_GENERALERROR + "Argument must be of type Number");
                }
            }
...
APDU.prototype.fromByteString = function(bs) {
    if (bs.length < 4) {
        new Error("APDU" + APDU.SW_GENERALERROR + " Command APDU must be at least 4 bytes long");
    }
    ...
    if (bs.length > 4) {
        ...
        if ((n == 0) && (l > 0)) {
            extended = true;
            if (l < 2) {
                new Error("APDU " + APDU.SW_WRONGLENGTH + " Extended length APDU too short");
            }
        ...
        if (l > 0) { // Case 3s / Case 3e / Case 4s / Case 4e
            if (l < n) {
                new Error("APDU" + APDU.SW_WRONGLENGTH + "Invalid Lc in APDU");
            }
            ...
            if (l > 0) { // Case 4s / Case 4e
                ...
                if (extended) {
                    if (l < 1) {
                        new Error("APDU" + APDU.SW_WRONGLENGTH + "Invalid Le in extended APDU");
                    }
                    n = (n << 8) + bs.byteAt(i++);
                    l--;
                }
        ...
        if (l > 0) {
            new Error("APDU" + APDU.SW_WRONGLENGTH + "Too many bytes in APDU");
        }
    }
}
